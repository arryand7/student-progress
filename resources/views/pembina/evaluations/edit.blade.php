...
@section('page-title', 'Edit Evaluasi Mingguan')

@section('content')
<form method="POST" action="{{ route('pembina.evaluations.update', $evaluation) }}">
    @csrf
    @method('PUT')
    ...
    <div class="space-y-6">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    <div class="w-14 h-14 bg-primary-100 rounded-xl flex items-center justify-center mr-4">
                        <span class="text-primary-700 font-bold text-lg">{{ substr($evaluation->enrollment->user->name, 0, 2) }}</span>
                    </div>
                    <div>
                        <h2 class="text-xl font-bold text-gray-800">{{ $evaluation->enrollment->user->name }}</h2>
                        <p class="text-gray-500">{{ $evaluation->enrollment->subject->name }} - {{ $evaluation->enrollment->program->name }}</p>
                    </div>
                </div>
                <div class="text-right">
                    <p class="text-2xl font-bold text-primary-600">Week {{ $evaluation->week_number }}</p>
                    <p class="text-gray-500">{{ $evaluation->year }}</p>
                </div>
            </div>
        </div>
        ...
        @foreach($components as $index => $component)
            @php
                $detail = $existingDetails->get($component->id);
            @endphp
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input 
                                    ...
                                    max="100" 
                                    step="0.01"
                                    value="{{ old('components.' . $index . '.score', $detail->score ?? '') }}"
                        ...
                        <div>
                            ...
                            <input 
                                ...
                                min="0"
                                value="{{ old('components.' . $index . '.time_spent_minutes', $detail->time_spent_minutes ?? '') }}"
                                class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500"
                        ...
                        <div>
                            ...
                            <input 
                                ...
                                min="0"
                                value="{{ old('components.' . $index . '.total_questions', $detail->total_questions ?? '') }}"
                                class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500"
                        ...
                        <div>
                            ...
                            <input 
                                ...
                                min="0"
                                value="{{ old('components.' . $index . '.attempted_questions', $detail->attempted_questions ?? '') }}"
                                class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500"
                        ...
                        <div>
                            ...
                            <input 
                                ...
                                min="0"
                                value="{{ old('components.' . $index . '.correct_questions', $detail->correct_questions ?? '') }}"
                                class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500"
                        ...
                        <div>
                            ...
                            <input 
                                ...
                                value="{{ old('components.' . $index . '.notes', $detail->notes ?? '') }}"
                                class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500"
        ...
        <div class="flex items-center justify-end space-x-4">
            <a href="{{ route('pembina.evaluations.show', $evaluation) }}" class="px-6 py-3 text-gray-700 hover:text-gray-900">
                Batal
            </a>
